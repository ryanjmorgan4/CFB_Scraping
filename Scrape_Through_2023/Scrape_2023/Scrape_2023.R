...
#First, let's read in the current csv files (up through the 2022 Season).
...
Total_Team_History <- read.csv(file="/Users/Ryan/Desktop/CFB_DATA/Total_Team_History_2022.csv")



#Do some spot checking on the files you just read in

Game_Logs %>% filter(Team == "nebraska", Season == 2022)
Game_Results %>% filter(Team == "nebraska", Season == 2022)
Individual_Season_Results %>% filter(Team == "nebraska")
Season_Averages %>% filter(Team == "nebraska", Season == 2022)
Total_Team_History %>% filter(Team == "nebraska")


#We now have everything as of 2022, so we now want to update it to be up through the 2023 Season.
...
read_team_stats(team="nebraska", year = 2023)


#get a list of teams who played in 2023:
  

teams_2023 <- Individual_Season_Results_Updated %>% filter(Year == 2023)

teams_2023 <- teams_2023$Team

teams_2023 <- nameFormat(teams_2023)




update_2023 <- function(teams){
  
  numteams = length(teams)
  
  output = read_team_stats(teams[1], 2023)
  
  i=2
  
  for(i in 2:numteams){
    
    output <- bind_rows(output, read_team_stats(teams[i], 2023))
    print(teams[i])
  }
...
season_averages_2023 <- update_2023(teams_2023)
length(unique(season_averages_2023$Team))
...
team_schedule_update_2023<- function(teams){
  
  numteams = length(teams)
  
  output = read_schedule_results(teams[1], 2023)
  
  i=2
  
  for(i in 2:numteams){
    output <- rbind(output, read_schedule_results(teams[i], 2023))
    print(teams[i])
...
schedule_results_2023 <- team_schedule_update_2023(teams_2023)
length(unique(schedule_results_2023$Team))
...
mult_team_offenseLogs_2023 <- function(teams){
  numteams = length(teams)
  
  output = OffensegameLogs(teams[1],2023)
  
  i=2
  
  for(i in 2:numteams){
    output <- bind_rows(output, OffensegameLogs(teams[i],2023))
    print(teams[i])
...
offense_2023_logs <- mult_team_offenseLogs_2023(teams_2023)
length(unique(offense_2023_logs$Team))

offense_2023_logs_save <- offense_2023_logs

offense_2023_logs <- offense_2023_logs_save
...
opponents_2023<-unique(offense_2023_logs$Opponent)

problemOpponents_2023 <- sort(opponents_2023[!opponents_2023 %in%  nameFormat(teams_2023)])
...
problemOpponents_2023 <- sort(opponents_2023[!opponents_2023 %in% teams_2023])

problemGames_2023 <- offense_2023_logs %>% filter(Opponent %in% problemOpponents_2023)

dim(problemGames_2023)


manually_Enter_2023 <- problemGames_2023

manually_Enter_2023$Team = problemGames_2023$Opponent

manually_Enter_2023$Opponent = problemGames_2023$Team

manually_Enter_2023$Points_Scored = problemGames_2023$Opp_points

manually_Enter_2023$Opp_points = problemGames_2023$Points_Scored


## Flip Locations ##
temp= problemGames_2023$Location

temp[problemGames_2023$Location == "Home"] = "Away"

temp[problemGames_2023$Location =="Away"] = "Home"

manually_Enter_2023$Location = temp

## Flip Result ##

temp= problemGames_2023$Result

temp[problemGames_2023$Result=="W"] = "L"

temp[problemGames_2023$Result =="L"] = "W"

manually_Enter_2023$Result = temp


manually_Enter_2023[,9:29] = NA
...
write.csv(manually_Enter_2023,"manual_Games2023.csv")
...
ManualGames_2023 <- read.csv("manual_Games2023_completed.csv")

ManualGames_2023$Date = mdy(ManualGames_2023$Date)

ManualGames_2023$Team = nameFormat(ManualGames_2023$Team)

ManualGames_2023$Opponent = nameFormat(ManualGames_2023$Opponent)



#Now bind it up:

offense_2023_logs <- bind_rows(offense_2023_logs, ManualGames_2023)

offense_2023_logs <- unique(offense_2023_logs)

read_defense_log <- function(team, year, offenselogs = offense_2023_logs){
...
All_Defense_Log <- function(teamList, offenseLogs= OffenseLogs){
  ...
  for(i in 1:numTeams){
    new = read_defense_log(teamList[i],2023)
...
defense_2023_logs <- All_Defense_Log(teams_2023)
...
gameLogs_2023 <- gameLogs_2023 %>% arrange(Team, Date)
...
anti_join(schedule_results_2023,gameLogs_2023)


#Replace liu opponent with long-island

schedule_results_2023$Opponent = gsub("^liu$", "long-island", schedule_results_2023$Opponent)
...
gameLogs_2023$Opponent = gsub("^houston-christian$", "houston-baptist", gameLogs_2023$Opponent)

anti_join(schedule_results_2023,gameLogs_2023)



colnames(schedule_results_2023) <- c("Team","Season","Conf","Game_Number","Date","Day","Location","TeamRank","Opponent","OpponentRank","OpponentConf","Result","Points_Scored","Opponent_Points_Scored","CurrentWins","CurrentLosses","CurrentStreak","Notes")

schedule_results_2023$Date <- ymd(schedule_results_2023$Date)

gameLogs_2023 <- left_join(schedule_results_2023, gameLogs_2023)
...
write.csv(file = "Total_Team_History_2023.csv",Total_Team_History, row.names = F)
...
colnames(season_averages_2023)[c(6:16,18:22)]<-c("Pass_Comp","Pass_Att","Pass_Pct","Pass_Yds","Pass_TD","Rush_Att","Rush_Yds","Rush_Avg","Rush_TD","Total_Plays","Total_Yds","Pass_First_Down","Rush_First_Down","Penalty_First_Down","Total_First_Down","Penalties")

which(is.na.data.frame(season_averages_2023),arr.ind=TRUE)

#Lets replace the NA values in the Games column with 0. 

season_averages_2023$Games[is.na(season_averages_2023$Games)] <- 0

table(which(is.na.data.frame(season_averages_2023),arr.ind=TRUE)[,2])


#Fix PAC conference

newConfs <- season_averages_2023$Conference
...
season_averages_2023$Conference = newConfs



#Sort

season_averages_updated <- rbind(Season_Averages, season_averages_2023)
...
colnames(schedule_results_2023)[c(8,10,11,13:17)]<- c("Team_Rank","Opponent_Rank","Opponent_Conference","Points_Scored","Opponent_Points_Scored","Current_Wins","Current_Losses","Current_Streak")


which(is.na.data.frame(schedule_results_2023),arr.ind=TRUE)

#Change columns 8 and 10, so that the NA values are "Unranked"

schedule_results_2023$Team_Rank[is.na(schedule_results_2023$Team_Rank)] <- "Unranked"

schedule_results_2023$Team_Rank <- as.factor(schedule_results_2023$Team_Rank)


schedule_results_2023$Opponent_Rank[is.na(schedule_results_2023$Opponent_Rank)] <- "Unranked"

schedule_results_2023$Opponent_Rank <- as.factor(schedule_results_2023$Opponent_Rank)


which(is.na.data.frame(schedule_results_2023),arr.ind=TRUE)


newResults <- schedule_results_2023$Result
...
schedule_results_2023$Result = newResults

#Fix Team Conference

colnames(schedule_results_2023)[3]= "Conference"
newConfs <- schedule_results_2023$Conference
...
schedule_results_2023$Conference = newConfs


#Fix Opponent Conference 

newConfs <- schedule_results_2023$Opponent_Conference
...
schedule_results_2023$Opponent_Conference = newConfs


Game_Results$Date = paste(Game_Results$Date)

schedule_results_2023$Date= paste(schedule_results_2023$Date)

Game_Results_Updated <- rbind(Game_Results, schedule_results_2023)
...
newResults <- gameLogs_2023$Result
...
gameLogs_2023$Result = newResults


#Fix Team Conference
colnames(gameLogs_2023) = colnames(Game_Logs)
newConfs <- gameLogs_2023$Conference
...
gameLogs_2023$Conference = newConfs


#Fix Opponent Conference 

newConfs <- gameLogs_2023$Opponent_Conference
...
gameLogs_2023$Opponent_Conference = newConfs


gameLogs_2023$Date = paste(gameLogs_2023$Date)
Game_Logs$Date = paste(Game_Logs$Date)


table(which(is.na.data.frame(gameLogs_2023),arr.ind=TRUE)[,2])



gameLogs_2023$Team_Rank[is.na(gameLogs_2023$Team_Rank)] <- "Unranked"

gameLogs_2023$Team_Rank <- as.factor(gameLogs_2023$Team_Rank)


gameLogs_2023$Opponent_Rank[is.na(gameLogs_2023$Opponent_Rank)] <- "Unranked"

gameLogs_2023$Opponent_Rank <- as.factor(gameLogs_2023$Opponent_Rank)


gameLogs_2023$Notes[is.na(gameLogs_2023$Notes)] <- ""



gameLogs_2023[which(is.na.data.frame(gameLogs_2023),arr.ind=TRUE)] = 0



#BIND

Game_Logs_Updated <- rbind(Game_Logs, gameLogs_2023)
