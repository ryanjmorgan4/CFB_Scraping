...
#First, let's read in the current csv files (up through the 2020 Season).

Game_Logs <- read.csv(file="C:\\Users\\Ryan\\Desktop\\CFB_DATA\\Game_Logs_Updated.csv", stringsAsFactors = F)

Game_Results <- read.csv(file="C:\\Users\\Ryan\\Desktop\\CFB_DATA\\Game_Results_Updated.csv", stringsAsFactors = F) %>% select(-X)

Individual_Season_Results <- read.csv(file="C:\\Users\\Ryan\\Desktop\\CFB_DATA\\Individual_Season_Results_updated.csv")


Season_Averages <- read.csv(file="C:\\Users\\Ryan\\Desktop\\CFB_DATA\\Season_Averages_Updated.csv") %>% unique()

Total_Team_History <- read.csv(file="C:\\Users\\Ryan\\Desktop\\CFB_DATA\\Total_Team_History_2020.csv")



#Do some spot checking on the files you just read in

Game_Logs %>% filter(Team == "nebraska", Season == 2020)
Game_Results %>% filter(Team == "nebraska", Season == 2020)
Individual_Season_Results %>% filter(Team == "nebraska")
Season_Averages %>% filter(Team == "nebraska", Season == 2020)
Total_Team_History %>% filter(Team == "nebraska")


#We now have everything as of 2020, so we now want to update it to be up through the 2021 Season.
...
#Overwrite any of the "Last_Year" = 2022 to 2021, since we don't have 2022 data yet.
#In addition (for now), filter out james-madison, since 2022 will be their first season, so there isn't any data yet

Total_Team_History$Last_Year = gsub(2022, 2021, Total_Team_History$Last_Year)
Total_Team_History = Total_Team_History %>% filter(Team != "james-madison")
...
read_school_history <- function(team){
  ...
  #if first date is 2022, then drop the first 18 entries
  
  if(schoolHistory[1] == "2022"){
    schoolHistory = schoolHistory[-c(1:18)]
  }
...
#I then ran the mult_history function on the TeamList list of 298 teams.
...
read_team_stats(team="nebraska", year = 2021)


#get a list of teams who played in 2021:
  


teams_2021 <- Total_Team_History %>% filter(Last_Year == 2021)

teams_2021 <- teams_2021$Team

teams_2021 <- nameFormat(teams_2021)




update_2021 <- function(teams){
  
  numteams = length(teams)
  
  output = read_team_stats(teams[1], 2021)
  
  i=2
  
  for(i in 2:numteams){
    
    output <- bind_rows(output, read_team_stats(teams[i], 2021))
...
season_averages_2021 <- update_2021(teams_2021)
...
team_schedule_update<- function(teams){
  ...
  output = read_schedule_results(teams[1], 2021)
  
  i=2
  
  for(i in 2:numteams){
    output <- rbind(output, read_schedule_results(teams[i], 2021))
...
schedule_results_2021 <- team_schedule_update(teams_2021)
...
mult_team_offenseLogs_2021 <- function(teams){
  numteams = length(teams)
  
  output = OffensegameLogs(teams[1],2021)
  
  i=2
  
  for(i in 2:numteams){
    output <- bind_rows(output, OffensegameLogs(teams[i],2021))
...
offense_2021_logs <- mult_team_offenseLogs_2021(teams_2021)

offense_2021_logs_save <- offense_2021_logs

offense_2021_logs <- offense_2021_logs_save



#Now the defense logs part:

opponents_2021<-unique(offense_2021_logs$Opponent)

problemOpponents <- sort(opponents_2021[!opponents_2021 %in%  nameFormat(teams_2021)])
...
problemOpponents <- sort(opponents_2021[!opponents_2021 %in% teams_2021])

problemGames <- offense_2021_logs %>% filter(Opponent %in% problemOpponents)
...
write.csv(manually_Enter,"manual_Games2021.csv")
...
ManualGames <- read.csv("C:\\Users\\Ryan\\Desktop\\CFB_DATA\\Scrape_Through_2021\\Scrape_2021\\manual_Games2021_finished.csv")
...
offense_2021_logs <- bind_rows(offense_2021_logs, ManualGames)

offense_2021_logs <- unique(offense_2021_logs)

read_defense_log <- function(team, year, offenselogs = offense_2021_logs){
...
All_Defense_Log <- function(teamList, offenseLogs= OffenseLogs){
  ...
  for(i in 1:numTeams){
    new = read_defense_log(teamList[i],2021)
...
defense_2021_logs <- All_Defense_Log(teams_2021)
...
gameLogs_2021 <- gameLogs_2021 %>% arrange(Team, Date)
...
anti_join(schedule_results_2021,gameLogs_2021)

#Replace liu opponent with long-island

schedule_results_2021$Opponent = gsub("^liu$", "long-island", schedule_results_2021$Opponent)

anti_join(schedule_results_2021,gameLogs_2021)



colnames(schedule_results_2021) <- c("Team","Season","Conf","Game_Number","Date","Day","Location","TeamRank","Opponent","OpponentRank","OpponentConf","Result","Points_Scored","Opponent_Points_Scored","CurrentWins","CurrentLosses","CurrentStreak","Notes")

schedule_results_2021$Date <- ymd(schedule_results_2021$Date)

gameLogs_2021 <- left_join(schedule_results_2021, gameLogs_2021)
...
#All of those NA's are for the Bowl winning percentage. Change the NA's to 0's.
...
write.csv(file = "Total_Team_History_2021.csv",Total_Team_History, row.names = F)
...
colnames(season_averages_2021)[c(6:16,18:22)]<-c("Pass_Comp","Pass_Att","Pass_Pct","Pass_Yds","Pass_TD","Rush_Att","Rush_Yds","Rush_Avg","Rush_TD","Total_Plays","Total_Yds","Pass_First_Down","Rush_First_Down","Penalty_First_Down","Total_First_Down","Penalties")

which(is.na.data.frame(season_averages_2021),arr.ind=TRUE)

#Lets replace the NA values in the Games column with 0. 

season_averages_2021$Games[is.na(season_averages_2021$Games)] <- 0

table(which(is.na.data.frame(season_averages_2021),arr.ind=TRUE)[,2])


#Fix PAC conference

newConfs <- season_averages_2021$Conference
...
season_averages_2021$Conference = newConfs



#Sort

season_averages_updated <- rbind(Season_Averages, season_averages_2021)
...
colnames(schedule_results_2021)[c(8,10,11,13:17)]<- c("Team_Rank","Opponent_Rank","Opponent_Conference","Points_Scored","Opponent_Points_Scored","Current_Wins","Current_Losses","Current_Streak")


which(is.na.data.frame(schedule_results_2021),arr.ind=TRUE)

#Change columns 8 and 10, so that the NA values are "Unranked"

schedule_results_2021$Team_Rank[is.na(schedule_results_2021$Team_Rank)] <- "Unranked"

schedule_results_2021$Team_Rank <- as.factor(schedule_results_2021$Team_Rank)


schedule_results_2021$Opponent_Rank[is.na(schedule_results_2021$Opponent_Rank)] <- "Unranked"

schedule_results_2021$Opponent_Rank <- as.factor(schedule_results_2021$Opponent_Rank)


which(is.na.data.frame(schedule_results_2021),arr.ind=TRUE)


newResults <- schedule_results_2021$Result
...
schedule_results_2021$Result = newResults


#Fix Team Conference

colnames(schedule_results_2021)[3]= "Conference"
newConfs <- schedule_results_2021$Conference
...
schedule_results_2021$Conference = newConfs


#Fix Opponent Conference 

newConfs <- schedule_results_2021$Opponent_Conference
...
schedule_results_2021$Opponent_Conference = newConfs


Game_Results$Date = paste(Game_Results$Date)

schedule_results_2021$Date= paste(schedule_results_2021$Date)

Game_Results_Updated <- rbind(Game_Results, schedule_results_2021)
...
newResults <- gameLogs_2021$Result
...
gameLogs_2021$Result = newResults


#Fix Team Conference
colnames(gameLogs_2021) = colnames(Game_Logs)
newConfs <- gameLogs_2021$Conference
...
gameLogs_2021$Conference = newConfs


#Fix Opponent Conference 

newConfs <- gameLogs_2021$Opponent_Conference
...
gameLogs_2021$Opponent_Conference = newConfs


gameLogs_2021$Date = paste(gameLogs_2021$Date)
Game_Logs$Date = paste(Game_Logs$Date)


table(which(is.na.data.frame(gameLogs_2021),arr.ind=TRUE)[,2])



gameLogs_2021$Team_Rank[is.na(gameLogs_2021$Team_Rank)] <- "Unranked"

gameLogs_2021$Team_Rank <- as.factor(gameLogs_2021$Team_Rank)


gameLogs_2021$Opponent_Rank[is.na(gameLogs_2021$Opponent_Rank)] <- "Unranked"

gameLogs_2021$Opponent_Rank <- as.factor(gameLogs_2021$Opponent_Rank)


gameLogs_2021$Notes[is.na(gameLogs_2021$Notes)] <- ""



gameLogs_2021[which(is.na.data.frame(gameLogs_2021),arr.ind=TRUE)] = 0



#BIND

Game_Logs_Updated <- rbind(Game_Logs, gameLogs_2021)
