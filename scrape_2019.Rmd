...
First, let's read in the current csv files (up through the 2018 Season).
...
Total_Team_History <- read.csv(file="C:\\Users\\Ryan\\Desktop\\CFB_DATA\\Total_Team_History_2018.csv")
```


We now have everything as of 2018, so we now want to update it to be up through the 2019 Season.
...
read_team_stats(team="nebraska", year = 2019)
```

get a list of teams who played in 2019:

```{r}

teams_2019 <- Total_Team_History %>% filter(Last_Year == 2019)

teams_2019 <- teams_2019$Team

teams_2019 <- nameFormat(teams_2019)
...
update_2019 <- function(teams){
  
  numteams = length(teams)
  
  output = read_team_stats(teams[1], 2019)
  
  i=2
  
  for(i in 2:numteams){
    
    output <- bind_rows(output, read_team_stats(teams[i], 2019))
...
season_averages_2019 <- update_2019(teams_2019)
```
...
team_schedule_update<- function(teams){
  ...
  output = read_schedule_results(teams[1], 2019)
  
  i=2
  
  for(i in 2:numteams){
    output <- rbind(output, read_schedule_results(teams[i], 2019))
...
schedule_results_2019 <- team_schedule_update(teams_2019)
```
...
```{r}
mult_team_offenseLogs_2019 <- function(teams){
    numteams = length(teams)
  
  output = OffensegameLogs(teams[1],2019)
  
  i=2
  
  for(i in 2:numteams){
    output <- bind_rows(output, OffensegameLogs(teams[i],2019))
...
offense_2019_logs <- mult_team_offenseLogs_2019(teams_2019)

offense_2019_logs_save <- offense_2019_logs

offense_2019_logs <- offense_2019_logs_save
```
...
```{r}
opponents_2019<-unique(offense_2019_logs$Opponent)

problemOpponents <- sort(opponents_2019[!opponents_2019 %in%  nameFormat(teams_2019)])
...
```{r}
offense_2019_logs %>% filter(Opponent == "")
...
problemOpponents <- sort(opponents_2019[!opponents_2019 %in% teams_2019])

problemGames <- offense_2019_logs %>% filter(Opponent %in% problemOpponents)

dim(problemGames)

```

so there are 115 bad games that I need to fix. Okay. Fine. 
...
```{r}
write.csv(manually_Enter,"manual_Games2019.csv")
```
...
ManualGames <- read.csv("C:\\Users\\Ryan\\Desktop\\CFB_DATA\\manual_Games2019_finished.csv")
...
```{r}
offense_2019_logs$Opponent = gsub(pattern = "unc-charlotte",replacement = "charlotte",x = offense_2019_logs$Opponent)
ManualGames$Team = gsub(pattern="unc-charlotte", replacement="charlotte", x=ManualGames$Team)


offense_2019_logs$Opponent[offense_2019_logs$Opponent == "louisiana"] = "louisiana-lafayette"
...
Now bind it up:
```{r}
offense_2019_logs <- bind_rows(offense_2019_logs, ManualGames)

offense_2019_logs <- unique(offense_2019_logs)
```


```{r}
read_defense_log <- function(team, year, offenselogs = offense_2019_logs){
...
All_Defense_Log <- function(teamList, offenseLogs= OffenseLogs){
  ...
  for(i in 1:numTeams){
    new = read_defense_log(teamList[i],2019)
...
defense_2019_logs <- All_Defense_Log(teams_2019)
...
```{r}
offense_2019_logs%>%filter(Team == "louisiana-lafayette", Opponent == "tulane")

defense_2019_logs%>%filter(Team == "louisiana-lafayette", Opponent == "tulane")


offense_2019_logs%>%filter(Team == "tulane", Opponent == "lousiana-lafayette")
defense_2019_logs%>%filter(Team == "tulane")

gameLogs_2019 %>% filter(Team == "tulane", Opponent == "louisiana-lafayette")

gameLogs_2019 %>% filter(Opponent == "tulane", Team == "louisiana-lafayette")
...
```{r}
gameLogs_2019 <- left_join(offense_2019_logs, defense_2019_logs)
...
```{r}
dim(offense_2019_logs)

dim(unique(offense_2019_logs))

dim(defense_2019_logs)

dim(unique(defense_2019_logs))

anti_join(offense_2019_logs, defense_2019_logs)

OLog <- offense_2019_logs[,7:29]

DLog <- defense_2019_logs[,5:25]
...
gameLogs_2019 <- gameLogs_2019 %>% arrange(Team, Date)
...
```{r}
anti_join(schedule_results_2019,gameLogs_2019)


schedule_results_2019 %>% filter(Team == "baylor")

gameLogs_2019 %>% filter(Team == "baylor")

schedule_results_2019 %>% filter(Team == "texas-tech")

gameLogs_2019 %>% filter(Team == "texas-tech")


```
```{r}

colnames(schedule_results_2019) <- c("Team","Season","Conf","Game_Number","Date","Day","Location","TeamRank","Opponent","OpponentRank","OpponentConf","Result","Points_Scored","Opponent_Points_Scored","CurrentWins","CurrentLosses","CurrentStreak","Notes")

schedule_results_2019$Date <- ymd(schedule_results_2019$Date)

gameLogs_2019 <- left_join(schedule_results_2019, gameLogs_2019)
```
...
write.csv(file = "Total_Team_History_2019.csv",Total_Team_History, row.names = F)
...
#In rows 12071, 12072, and 12843, there isn't a column 8 or 9

Individual_Season_Results[12071, ]
Individual_Season_Results[12072, ]
Individual_Season_Results[12843, ]
...
```{r}
colnames(season_averages_2019)[c(6:16,18:22)]<-c("Pass_Comp","Pass_Att","Pass_Pct","Pass_Yds","Pass_TD","Rush_Att","Rush_Yds","Rush_Avg","Rush_TD","Total_Plays","Total_Yds","Pass_First_Down","Rush_First_Down","Penalty_First_Down","Total_First_Down","Penalties")

which(is.na.data.frame(season_averages_2019),arr.ind=TRUE)

#Lets replace the NA values in the Games column with 0. 

season_averages_2019$Games[is.na(season_averages_2019$Games)] <- 0

table(which(is.na.data.frame(season_averages_2019),arr.ind=TRUE)[,2])


#Fix PAC conference

newConfs <- season_averages_2019$Conference
...
season_averages_2019$Conference = newConfs



#Sort

season_averages_updated <- rbind(Season_Averages, season_averages_2019)
...
colnames(schedule_results_2019)[c(8,10,11,13:17)]<- c("Team_Rank","Opponent_Rank","Opponent_Conference","Points_Scored","Opponent_Points_Scored","Current_Wins","Current_Losses","Current_Streak")


which(is.na.data.frame(schedule_results_2019),arr.ind=TRUE)

#Change columns 8 and 10, so that the NA values are "Unranked"

schedule_results_2019$Team_Rank[is.na(schedule_results_2019$Team_Rank)] <- "Unranked"

schedule_results_2019$Team_Rank <- as.factor(schedule_results_2019$Team_Rank)


schedule_results_2019$Opponent_Rank[is.na(schedule_results_2019$Opponent_Rank)] <- "Unranked"

schedule_results_2019$Opponent_Rank <- as.factor(schedule_results_2019$Opponent_Rank)
...
```{r}
newResults <- schedule_results_2019$Result
...
schedule_results_2019$Result = newResults


#Fix Team Conference

colnames(schedule_results_2019)[3]= "Conference"
newConfs <- schedule_results_2019$Conference
...
schedule_results_2019$Conference = newConfs


#Fix Opponent Conference 

newConfs <- schedule_results_2019$Opponent_Conference
...
schedule_results_2019$Opponent_Conference = newConfs


Game_Results$Date = paste(Game_Results$Date)

schedule_results_2019$Date= paste(schedule_results_2019$Date)

Game_Results_Updated <- rbind(Game_Results, schedule_results_2019)
...
```{r}
newResults <- gameLogs_2019$Result
...
gameLogs_2019$Result = newResults


#Fix Team Conference
colnames(gameLogs_2019) = colnames(Game_Logs)
newConfs <- gameLogs_2019$Conference
...
gameLogs_2019$Conference = newConfs


#Fix Opponent Conference 

newConfs <- gameLogs_2019$Opponent_Conference
...
gameLogs_2019$Opponent_Conference = newConfs


gameLogs_2019$Date = paste(gameLogs_2019$Date)
...
Game_Logs_Updated <- rbind(Game_Logs, gameLogs_2019)
